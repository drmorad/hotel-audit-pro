...
import { MOCK_USERS } from '../constants'; // Using MOCK_USERS for dynamic assignee list
...
const IncidentList: React.FC<IncidentListProps> = ({ 
  ...
  // Use ID reference instead of object snapshot to prevent stale data
  const [selectedIncidentId, setSelectedIncidentId] = useState<string | null>(null);

  // Derived state for the active incident ensures it is always fresh
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 transition-colors">
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {filteredAndSortedIncidents.map((incident) => (
                <tr key={incident.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onClick={() => setSelectedIncidentId(incident.id)}>
        ...
        <div className="md:hidden space-y-4">
          {filteredAndSortedIncidents.map((incident) => (
            <div key={incident.id} onClick={() => setSelectedIncidentId(incident.id)} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow bg-white dark:bg-gray-800 cursor-pointer">
